...
QPU_FOUR = [
    ...
    {"0": 453, "1": 547},
    {"0": 148, "1": 852},
]  # ibm_kingston, qubit 0, job dal14kot9ckc739kv7n0
...
ax.set_xticks([-1, 0, 1])
ax.set_yticks([-1, 0, 1])
fig.savefig(OUT / "unit_circle.pdf")
plt.close(fig)
...
ax.set_ylim(0, 620)
fig.savefig(OUT / "plus_sampled.pdf")
plt.close(fig)
...
fig.savefig(OUT / "plus_minus_bases.pdf")
plt.close(fig)
...
fig.savefig(OUT / "bloch_sphere.pdf")
plt.close(fig)
...
fig.savefig(OUT / "bloch_psi_qiskit.pdf")
plt.close(fig)
...
def path_panel(ax, mid_gate, amps_first, amps_second, title):
    ...
    for (x, y), lab in nodes.items():
        ax.text(
            ...
            va="center",
            fontsize=11,
            bbox=dict(boxstyle="circle,pad=0.25", fc="white", ec=GRAY, lw=0.8),
        )
...
path_panel(
    ...
    [r"$\frac{1}{\sqrt{2}}$", r"$-\frac{1}{\sqrt{2}}$"],
    r"$HH|0\rangle$: the two paths cancel",
)
path_panel(
    axes[1],
    r"$Z$, then second $H$",
    [r"$\frac{1}{\sqrt{2}}$", r"$\frac{1}{\sqrt{2}}$"],
    [r"$\frac{1}{\sqrt{2}}$", r"$+\frac{1}{\sqrt{2}}$"],
    r"$HZH|0\rangle$: the two paths add",
)
fig.savefig(OUT / "interference_paths.pdf")
plt.close(fig)
hh = QuantumCircuit(1)
hh.h(0)
hh.h(0)
...
for ax, label, ex, sc, qcnt in zip(axes, LABELS, EXACT_FOUR, SIM_FOUR, QPU_FOUR):
    ...
    ax.bar_label(b1, padding=2, fontsize=6)
    ax.bar_label(b2, padding=2, fontsize=6)
    ax.set_xticks([0, 1], ["0", "1"])
    ax.set_title(label.replace("|+>", r"$|+\rangle$").replace("|->", r"$|-\rangle$").replace("after H", r"after $H$"))
    ax.set_ylim(0, 1180)
axes[0].set_ylabel(f"counts out of {SHOTS}")
axes[0].legend(frameon=False, fontsize=8)
fig.tight_layout()
fig.savefig(OUT / "hardware_four.pdf")
plt.close(fig)
...
print(
    "[7/7] hardware_four.pdf   readout matrix from circuits 3 and 4 (rows read 0/1, columns prepared 0/1):",
    np.round(M, 3).tolist(),
)
print(
    "      a fair state would read as P(1) =",
    round(float(p_fair[1]), 3),
    "; mitigated P(0) for the direct runs:",
    [round(float(m[0]), 3) for m in mit],
)
