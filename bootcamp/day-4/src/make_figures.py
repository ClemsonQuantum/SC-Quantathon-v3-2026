...
def bars(ax, counts, color, label=None, offset=0.0, width=0.55):
    ...
    ax.bar_label(b, padding=2, fontsize=8); ax.set_xticks(np.arange(4), keys)
...
for label, (x, y, z, dx, dy, dz) in {r"$|0\rangle$": (0, 0, 1, 0, 0, 0.14), r"$|1\rangle$": (0, 0, -1, 0, 0, -0.16),
                                      r"$|+\rangle$": (1, 0, 0, 0.22, 0, -0.06), r"$|-\rangle$": (-1, 0, 0, -0.22, 0, 0.04),
                                      r"$|{+}i\rangle$": (0, 1, 0, 0.1, 0.3, -0.14)}.items():
...
p_path = np.array([bloch(Statevector([1, np.exp(1j * l)]) / np.sqrt(2)) for l in lam])
...
for t in thetas:
    ...
    qc.measure_all(); p1_sampled.append(counts_of(qc).get("1", 0) / SHOTS)
...
def measure_in_basis(prep, basis):
    ...
    qc.measure_all(); return counts_of(qc)
prep = QuantumCircuit(1); prep.h(0); prep.s(0)
...
exact_z = sv.probabilities()[0]; exact_x = abs(Statevector.from_label("+").inner(sv)) ** 2
print("      predict-then-measure: exact P(0)_Z =", round(float(exact_z), 4), " measured", NB_PREDICT["z"],
      " | exact P(+) =", round(float(exact_x), 4), " measured", NB_PREDICT["x"], " | R_y(2pi/3) single run:", NB_RY)
...
def measure_both(prep, basis):
    ...
    qc.measure_all(); return counts_of(qc)
plus_plus = QuantumCircuit(2); plus_plus.h(0); plus_plus.h(1)
...
for i, (name, prep) in enumerate([("Bell state", bell), (r"$|+\rangle|+\rangle$", plus_plus)]):
    for j, basis in enumerate("ZX"):
        c = NB_CORR[(name, basis)]; res[(name, basis)] = c
        bars(axes[i, j], c, BLUE if i == 0 else ORANGE)
        axes[i, j].set_title(f"{name}, ${basis}{basis}$ basis"); axes[i, j].set_ylim(0, 1150)
...
ax.axhline(16 * 2 ** 30, ls="--", lw=0.8, color=GRAY); ax.text(1.5, 16 * 2 ** 30 * 2.2, "16 GB laptop", fontsize=8, color=GRAY)
