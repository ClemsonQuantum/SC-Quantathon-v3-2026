...
fig.savefig(OUT / "qrng_circuit.pdf")
plt.close(fig)
...
ax.set_ylim(0, 620)
fig.savefig(OUT / "qrng_simulator.pdf")
plt.close(fig)
...
ax.plot(
    list(P_AT_NOTEBOOK),
    [abs(v - 0.5) for v in P_AT_NOTEBOOK.values()],
    "s",
    ms=6,
    color=ORANGE,
    label="the notebook's four runs",
)
ax.plot(shots_list, 0.5 / np.sqrt(shots_list), "-", color=GRAY, label=r"one standard deviation, $\sqrt{p(1-p)/N}$")
ax.set_xscale("log")
ax.set_yscale("log")
ax.set_xlabel("shots $N$")
ax.set_ylabel("error in the estimate of $P(0)$")
ax.legend(frameon=False, loc="lower left")
fig.savefig(OUT / "shots_convergence.pdf")
plt.close(fig)
...
rows = [
    ("classical coin", coin_counts, GREEN),
    ("AerSimulator", sim_counts, BLUE),
    ("ibm_kingston", LIVE_COUNTS, ORANGE),
]
x = np.arange(2)
w = 0.26
