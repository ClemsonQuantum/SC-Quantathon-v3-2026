...
Two lessons hide in the table. Calibration data are measurements, not specifications: $T_2$ is bounded not by $T_1$ but by $2T_1$, so a snapshot reporting $T_2 > 2T_1$ would be a measurement artefact rather than physics (this one reports none), and one qubit reports a 50\% error and a few couplers 100\%, which means ``currently unusable'' rather than a number to plug in. And the time scales are separated by two orders of magnitude: a two-qubit gate takes half a microsecond against coherence times of hundreds of microseconds, so a circuit of depth twenty runs in about ten microseconds and loses only a few percent to decoherence. For short circuits the two-qubit gate error, at roughly a percent per gate, matters first.
...
Two things are worth reading off. Readout mitigation moves this run only a little, because the pair's readout is already at the percent level: it takes the impossible outcomes from 1.0\% to 0.6\%, so four of the ten shots were readout and the six that survive came from inside the circuit, the \code{cz} gate and relaxation during it. (The tilt between \code{00} and \code{11}, 527 against 463, is about two $\sigma$, which a fair split produces one run in twenty, so it cannot be attributed to anything.) What survives the readout matrix is exactly what zero-noise extrapolation is for: the runtime's TREX plus ZNE takes $\braket{ZZ}$ from $0.980$ to $0.995$, three quarters of the way to the exact value, and it did so by running folded copies of the circuit and fitting, as Section~6 did on the snapshot. The remaining half percent is the price of extrapolating from noisy points. A second lesson is in the pair itself: the same two qubits gave 1.0\% impossible outcomes on Day~4 and 1.0\% here, an hour apart, while qubit~0 of the same chip was misreading half its zeros on Days~1 and 2. Which qubits a job lands on matters more than which machine.
...
\item \code{AerSimulator.from\_backend} combines the three from calibration data; for the Bell state its impossible-outcome fraction lands within two percentage points of the device's (3.0\% against 1.0\%), with the current processor beating the retired snapshot (Table~\ref{tab:three}).
