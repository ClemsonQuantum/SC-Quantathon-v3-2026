...
print(
    f"          median T1 {np.median(t1):.0f} us, T2 {np.median(t2):.0f} us, readout {np.median(ro):.2%}; "
    f"{gate} median error {np.median(list(gerr.values())):.2%}, worst {max(gerr.values()):.1%}, duration {np.median(list(gdur.values())) * 1e9:.0f} ns"
)
bp = min(gerr, key=gerr.get)
print(f"          best pair {bp}: {gerr[bp]:.3%}; qubit 0: T1 {t1[0]:.0f} us, T2 {t2[0]:.0f} us, readout {ro[0]:.2%}")

# Figure 1: calibration data across the chip
fig, axes = plt.subplots(1, 3, figsize=(9, 2.6))
axes[0].hist(t1, bins=25, color=BLUE)
axes[0].set_xlabel("T1 (us)")
axes[0].set_ylabel("qubits")
axes[1].hist(ro * 100, bins=25, color=ORANGE)
axes[1].set_xlabel("readout error (%)")
axes[2].hist(np.array(list(gerr.values())) * 100, bins=25, color=GREEN)
axes[2].set_xlabel(f"{gate} gate error (%)")
fig.tight_layout()
...
print(
    f"[transpile] Bell: {dict(bell.count_ops())} depth {bell.depth()} -> {dict(bell_isa.count_ops())} depth {bell_isa.depth()}, physical {bell_isa.layout.final_index_layout()}"
)
...
for level in range(4):
    ...
    ops = isa.count_ops()
    print(
        f"            GHZ(5) level {level}: depth {isa.depth()}, {gate} {ops.get(gate, 0)}, gates {sum(v for k, v in ops.items() if k not in ('measure', 'barrier'))}"
    )
manila = FakeManilaV2()
...
print(
    f"            star on Manila line: cx {star_isa.count_ops().get('cx', 0)} for 4 logical CNOTs, depth {star_isa.depth()}"
)
...
waits = np.array(NB["waits"])
p1 = NB["p1_survive"]
pp = NB["p_plus"]  # the notebook's own runs
fig, axes = plt.subplots(1, 2, figsize=(7.4, 2.7))
axes[0].plot(waits * 1e6, p1, "o", color=BLUE, label="simulated, 1000 shots")
axes[0].plot(waits * 1e6, np.exp(-waits / T1), "-", color=GRAY, label=r"$e^{-t/T_1}$")
axes[0].set_xlabel("idle time (us)")
axes[0].set_ylabel(r"$P(1)$ after preparing $|1\rangle$")
axes[0].set_title(f"$T_1$ = {T1 * 1e6:.0f} us")
axes[0].legend(frameon=False)
axes[1].plot(waits * 1e6, pp, "o", color=ORANGE, label="simulated, 1000 shots")
axes[1].plot(waits * 1e6, 0.5 * (1 + np.exp(-waits / T2)), "-", color=GRAY, label=r"$\frac{1}{2}(1+e^{-t/T_2})$")
axes[1].set_xlabel("idle time (us)")
axes[1].set_ylabel(r"$P(+)$ after preparing $|+\rangle$")
axes[1].set_title(f"$T_2$ = {T2 * 1e6:.0f} us")
axes[1].legend(frameon=False)
fig.tight_layout()
fig.savefig(OUT / "decoherence.pdf")
plt.close(fig)
...
ax.set_xlabel("number of CNOTs $k$")
ax.set_ylabel("$P(00)$")
ax.legend(frameon=False)
fig.savefig(OUT / "cnot_decay.pdf")
plt.close(fig)
...
def imp(c):
    return (c.get("01", 0) + c.get("10", 0)) / sum(c.values())
...
for i, (label, c, color) in enumerate(rows):
    tot = sum(c.values())
    b = ax.bar(x + (i - 1) * w, [c.get(k, 0) / tot * 100 for k in keys], w, label=label, color=color)
...
fig.savefig(OUT / "bell_three_ways.pdf")
plt.close(fig)
for label, c, _ in rows:
    print(f"[4/8] {label:<16} {c}  impossible {imp(c):.1%}  fidelity {hellinger_fidelity(ideal, c):.3f}")
...
p_true = np.clip(np.linalg.solve(M, p_meas), 0, None)
p_true /= p_true.sum()
fig, ax = plt.subplots(figsize=(4.6, 2.8))
...
ax.bar_label(b1, labels=[f"{v * 100:.1f}" for v in p_meas], padding=2, fontsize=7)
ax.bar_label(b2, labels=[f"{v * 100:.1f}" for v in p_true], padding=2, fontsize=7)
ax.set_xticks(x, keys)
ax.set_xlabel("outcome")
ax.set_ylabel("percent")
ax.set_ylim(0, 60)
ax.legend(frameon=False, loc="upper center")
fig.savefig(OUT / "readout_mitigation.pdf")
plt.close(fig)
print(
    f"[5/8] readout_mitigation.pdf   physical qubits {phys}, M diag {np.round(np.diag(M), 3)}, impossible raw {imp(sim):.1%} -> mitigated {(p_true[1] + p_true[2]):.1%}"
)
print("      M =\n", np.round(M, 3))

# Figure 6: zero-noise extrapolation
scales = np.array(NB["noise_scales"])
vals = NB["zz_values"]  # the notebook's own run
...
ax.set_xlabel(r"noise scale $\lambda$")
ax.set_ylabel(r"$\langle ZZ\rangle$")
ax.legend(frameon=False, loc="lower left")
fig.savefig(OUT / "zne.pdf")
plt.close(fig)
print(
    f"[6/8] zne.pdf   <ZZ> at scales {scales}: {np.round(vals, 4)}; linear -> {np.polyval(lin, 0):.4f}, quadratic -> {np.polyval(quad, 0):.4f}"
)
...
fig.savefig(OUT / "hth_circuit.pdf")
plt.close(fig)
fig = native.draw("mpl", style="clifford")
fig.savefig(OUT / "hth_native.pdf")
plt.close(fig)
print(
    "[7/8] hth_circuit.pdf, hth_native.pdf   native gates:",
    dict(native.count_ops()),
    " depth",
    qc.depth(),
    "->",
    native.depth(),
...
ax.axhline(1, color=GRAY, lw=0.8)
ax.set_ylabel(r"$\langle ZZ\rangle$ of the Bell state")
ax.tick_params(axis="x", labelsize=8)
fig.savefig(OUT / "live_zz.pdf")
plt.close(fig)
print(
    "[8/8] live_zz.pdf   live counts",
    LIVE_COUNTS,
    "impossible",
    f"{imp(LIVE_COUNTS):.1%}",
    "fidelity",
    f"{hellinger_fidelity(ideal, LIVE_COUNTS):.3f}",
)
